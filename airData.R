...
calendarPlot(mydata, pollutant = "ozone", year=2003)

# timePlot. Seleccionar mes, año y si se puede site o mejor provincia.
timePlot(selectByDate(mydata, year = 2004, month = 03),
         pollutant = c("co", "no", "no2", "ozone", "pm10", "so2"), type = 'site')

# SmoothTrend OK. seleccionar pollutant. Y provincia
smoothTrend(mydata, pollutant = "ozone", ylab = "concentration (ppb)",
            main = "monthly mean o3")
# timeVariation OK. Seleccionar Provincia y en vez de por horas, como sale abajo.
timeVariation(mydata, pollutant = c("no", "co", "no2", "ozone"), normalise = TRUE)

# scatterPlot. controlar eje x e y, año y provincia.
scatterPlot(mydata, x = "no", y = "no2", method = "hexbin", col= "jet")
scatterPlot(selectByDate(mydata, year = 2003), x = "no", y = "no2",
            method = "density", col = "jet")
# otro ejemplo por estacion de año. 
scatterPlot(mydata, x = "no", y = "no2", z = "ozone", type = c("season", "weekend"),
            limits = c(0, 30))
...
linearRelation(mydata, x = "no", y = "so2")
linearRelation(mydata, x = "no", y = "ozone", period = "month")
linearRelation(mydata, x = "no", y = "ozone", period = "year")

# Mirar trendLevel para ver si se puede sacar algo.
trendLevel(mydata, pollutant = "ozone",
